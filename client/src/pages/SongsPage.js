  ...
  return (
    <Container>
      ...
      <Grid container spacing={6}>
        ...
        {/* ANSWER: Value label display is necessary if we want to see the current value of the slider. */}
        {/* ANSWER: It seems xs in total is 12 units so if we want all three on same tow xs = 4 */}
        <Grid item xs={4}>
          <p>Danceability</p>
          <Slider
            value={danceability}
      ...
      <Button onClick={() => {
        setDanceability([Math.random(), Math.random()].sort());
        setEnergy([Math.random(), Math.random()].sort());
        setValence([Math.random(), Math.random()].sort());
        search(); 
